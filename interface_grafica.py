...
if st.sidebar.button("Cadastrar Nova Pessoa", use_container_width=True): st.session_state.view = 'add_person'; st.session_state.person_to_edit = None
...
if 'list' in st.session_state.view:
    ...
    lista_atual = []
    if st.session_state.view == 'list_all': lista_atual = st.session_state.registro.getTodos()
    elif st.session_state.view == 'list_approved': lista_atual = st.session_state.registro.getAlunosPorStatus(aprovado=True)
    elif st.session_state.view == 'list_failed_grade': lista_atual = st.session_state.registro.getReprovadosPorMedia()
    elif st.session_state.view == 'list_failed_absence': lista_atual = st.session_state.registro.getReprovadosPorFalta()
    ...
    if matricula_para_editar:
        try:
            st.session_state.person_to_edit = st.session_state.registro.buscar(matricula_para_editar)
        except Exception:
    ...
    if matricula_para_remover:
        try:
            ...
            if st.button("Sim, remover", type="primary"):
                ...
                salvar_dados(); st.success(f"{pessoa.getNome()} foi removido(a)."); st.session_state.view = 'list_all'; st.rerun()
        except Exception as e:
            st.error(f"Erro: {e}")
